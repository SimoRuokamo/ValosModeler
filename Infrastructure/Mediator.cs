...
namespace ValosModeler.Infrastructure
{
	...
	/// collection.
	/// !! Registered objects must be unregistered from the mediator manually before they are destroyed. !!
	/// </summary>
	public class Mediator
	{
		...
		private bool NotifyColleagues(object key, object message)
		{
			...
			{
				Delegate action = wr[i].GetMethod();
		...
		public void NotifyColleaguesAsync<T>(string key, T message)
		{
			...
			Task.Run(() => NotifyColleagues(key, message));
#else
		...
		{
#if NET5_0_OR_GREATER
			Task.Run(() => NotifyColleagues( message));
#else
